...
class SelectionDialog(QDialog):   #選擇single/multiple
    ...
    def initUI(self):
        ...
        self.singleButton.clicked.connect(self.singleSelected)
...
class InputDialog(QDialog):  # single頁面
    def __init__(self):
        super().__init__()
        self.initUI() 
    ...
    def accept(self):
        try:
            row_data_lines, wafer_number, rowct, colct, lot = read_bak_file(self.fileName)
            converted_data, count_F, count_1, count_X= convert_row_data(row_data_lines)
            self.update_xml(converted_data, wafer_number, rowct, colct, lot, count_F, count_1, count_X)
            super().accept()
        except:
            import logging
    ...
    def update_xml(self,converted_data, wafer_number, rowct, colct, lot, count_F, count_1, count_X):
        ...
        substrate_id = lot[:6] + '-' + wafer_number + '-' + self.calculate_checksum(lot[:6] + '-' + wafer_number + '-')
...
class InputMultiDialog(QDialog): 
    ...
    def accept(self):
        try:
            ...
            for i, filename in enumerate(bak_files):
                ...
                converted_data, count_F, count_1, count_X = convert_row_data(row_data_lines)
                # 如果當前文件是第一個文件，則調用 self.update_xml
                # 否則，調用 self.update_multi_xml
                if i == 0:
                    self.update_xml(converted_data, wafer_number, rowct, colct, lot, current_time, count_F, count_1, count_X)
                    smallest_wafer_number = wafer_number 
                else:
                    self.update_multi_xml(converted_data, wafer_number, rowct, colct, lot, current_time, smallest_wafer_number, count_F, count_1, count_X)
            super().accept()
        except:
            import logging
            logging.basicConfig(filename='ParsingError.log', level=logging.DEBUG)
    ...
    def update_xml(self,converted_data, wafer_number, rowct, colct, lot, current_time, count_F, count_1, count_X):
        with open('template.xml', 'r') as file:
            xml_string = file.read()
        xml_string = xml_string.replace('ProductId="ACIPC60K0AA111"', f'ProductId="{self.productId.text()}"')
        xml_string = xml_string.replace('DeviceSizeX="180"', f'DeviceSizeX="{self.deviceSizeX.text()}"')
    ...
    def update_multi_xml(self,converted_data , wafer_number, rowct, colct, lot, current_time, smallest_wafer_number, count_F, count_1, count_X):
        with open('template _multi.xml', 'r') as file:
            xml_template_string = file.read()
        xml_template_string = xml_template_string.replace('ProductId="ACIPC60K0AA111"', f'ProductId="{self.productId.text()}"')
        xml_template_string = xml_template_string.replace('DeviceSizeX="180"', f'DeviceSizeX="{self.deviceSizeX.text()}"')
        xml_template_string = xml_template_string.replace('DeviceSizeY="240"', f'DeviceSizeY="{self.deviceSizeY.text()}"')
...
if __name__ == '__main__':
    ...
    if ex.exec_() == QDialog.Accepted:
        if ex.selection == "single":
            dialog = InputDialog()
            dialog.exec_()
        elif ex.selection == "multiple":
